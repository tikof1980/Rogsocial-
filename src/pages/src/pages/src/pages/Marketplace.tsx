...
export default function Marketplace() {
  ...
  return (
    <div className="p-4 relative">
      <button onClick={() => setShowCart(true)} className="absolute top-4 right-4 z-10">
      ...
      <h2 className="font-bold text-lg mb-4">Boutiques & produits</h2>
        ...
        {products.map(p => (
          <div key={p.id} className="border border-gray-200 dark:border-gray-800 rounded-xl overflow-hidden">
            ...
            <div className="p-2">
              ...
              <button onClick={() => addToCart(p)} className="mt-2 w-full bg-brand-500 text-white text-xs rounded-full py-1.5">
                Ajouter au panier
              </button>
            </div>
          </div>
      ...
      {showCart && (
        <div className="fixed inset-0 max-w-md mx-auto bg-black/50 z-40 flex items-end">
          <div className="bg-white dark:bg-gray-900 w-full rounded-t-2xl p-4 max-h-[80vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-3">
              <h3 className="font-semibold">Mon panier</h3>
              <button onClick={() => setShowCart(false)}><X size={20} /></button>
            </div>
            {cart.length === 0 && <p className="text-sm text-gray-500">Votre panier est vide.</p>}
            {cart.map(c => (
              <div key={c.product.id} className="flex items-center gap-3 mb-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-gray-500">Qté : {c.quantity}</p>
                  <p className="text-brand-500 text-sm font-bold">{(c.product.price * c.quantity).toLocaleString()} FCFA</p>
                </div>
                <button onClick={() => removeFromCart(c.product.id)} className="text-xs text-gray-400">Retirer</button>
              </div>
            ))}
            {cart.length > 0 && (
              <>
                ...
                <button
                  onClick={() => { cart.forEach(c => placeOrder(c.product, c.quantity)); setShowCart(false); }}
