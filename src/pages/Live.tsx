    ...
    <div className="fixed inset-0 z-40 mx-auto max-w-md h-[100dvh] bg-black text-white overflow-hidden">
      ...
      <div className="relative flex items-center justify-between p-4 pt-[max(1rem,env(safe-area-inset-top))]">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setViewingLiveId(null)}>
            <X size={22} />
          </button>
        </div>
      </div>
      ...
      {live.product && (
        <div className="absolute top-20 left-4 right-4 bg-white/95 text-black rounded-xl p-3 flex items-center gap-3 shadow-lg">
          ...
          <button className="bg-brand-500 text-white text-xs px-3 py-2 rounded-full font-semibold">
            Commander
          </button>
        </div>
      )}
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4 pb-[max(1rem,env(safe-area-inset-bottom))]">
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            className="flex-1 bg-black/40 rounded-full px-4 py-2 text-sm outline-none placeholder:text-gray-300"
          />
...
export default function Live() {
  ...
  if (viewingLiveId) {
    return <LiveRoom liveId={viewingLiveId} />;
  }
  ...
  return (
    <div className="p-4 pb-6 h-full overflow-y-auto">
      <button
        ...
        className="w-full bg-red-600 text-white rounded-2xl py-3.5 font-semibold mb-5 flex items-center justify-center gap-2 shadow-lg shadow-red-600/20 active:scale-[0.98] transition-transform"
      >
      ...
      <div className="grid grid-cols-2 gap-3">
        {liveStreams.map(l => {
          ...
          return (
            <button
              key={l.id}
              onClick={() => setViewingLiveId(l.id)}
              className="relative rounded-2xl overflow-hidden h-56 text-left border border-gray-200 dark:border-gray-800 shadow-sm"
            >
      ...
      <button onClick={() => setTab('feed')} className="mt-6 text-sm text-brand-500 w-full text-center">
        ← Retour au feed
      </button>
    </div>
  );
