...
function mockAIReply(prompt: string): string {
  if (/hashtag/i.test(prompt)) {
    return 'Voici des hashtags suggérés : #abidjan #madeinci #tendance #wax #rogsocial';
  }
  if (/script/i.test(prompt)) {
    return 'Script proposé : 1) Accroche 3s 2) Présentation produit 3) Preuve sociale 4) Appel à l\'action "Commander maintenant"';
  }
  if (/client/i.test(prompt)) {
    return 'Réponse suggérée : "Merci pour votre message ! Le produit est disponible, souhaitez-vous que je prenne votre commande ?"';
  }
  if (/stat/i.test(prompt)) {
    return 'Vos vidéos avec produit associé génèrent 2,3x plus d\'engagement que les autres cette semaine.';
  }
      ...
      <div className="flex-shrink-0 flex gap-2 overflow-x-auto px-4 pb-2">
        {suggestions.map(({ icon: Icon, label }) => (
          <button
            key={label}
            onClick={() => send(label)}
            className="flex items-center gap-1 text-xs bg-gray-100 dark:bg-gray-800 px-3 py-2 rounded-full whitespace-nowrap flex-shrink-0"
          >
            <Icon size={14} />
            {label}
          </button>
